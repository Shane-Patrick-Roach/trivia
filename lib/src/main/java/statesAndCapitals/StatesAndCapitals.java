...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(stateInfo -> stateInfo.getStateAnthem().split(" ").length == 1);
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream().collect(Collectors.averagingDouble(c -> c.getCapital().getAverageYearlyPrecipitationInInches()));
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().collect(Collectors.summingInt(c -> c.getCapital().getAverageYearlyPrecipitationInInches()));
        ...
        Map<String, Long> numberOfStatesByTimeZone = states.stream().collect(Collectors.groupingBy(c->c.getTimeZones().toString(), counting()));
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream().collect(Collectors.groupingBy(c->c.getCapital().getTimeZone().toString(), counting()));
        ...
        Comparator<StateInfo> stateInfoComparator = Comparator.comparing(StateInfo::getStateTree);

        List<String> stateTreesSortedAscending = states.stream().sorted(stateInfoComparator).map(StateInfo::getStateTree).toList();
        ...
        String allStateNamesSemicolonDelimited = states.stream().map(c->c.getStateName()).collect(joining("; "));
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream().map(StateInfo::getStateBird).distinct().filter(bird -> !bird.contains("mockingbird")).toList();
